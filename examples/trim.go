...
func addStream(name string, oc *gmf.FmtCtx, ist *gmf.Stream, ictx *gmf.FmtCtx) (int, int, error) {
	var (
		cc      *gmf.CodecCtx
		ost     *gmf.Stream
		options []gmf.Option
	...
	ictx.SeekFrameAt(trimStart, ist.Index())
...
func main() {
	...
	flag.StringVar(&src, "src", "", "source file, e.g.: -src=sample.mp4")
	flag.StringVar(&dst, "dst", "", "destination file, e.g. -dst=result.flv")
	flag.Parse()

	if src == "" || dst == "" {
		log.Fatal("source and destination required")
	}
	...
	if len(streamMap) == 0 {
		...
		if err != nil {
			log.Fatalf("No video stream found in %s", src)
		} else {
			i, o, err := addStream("libx264", octx, srcVideoStream, ictx)
			if err != nil {
				log.Fatal(err)
			}
			streamMap[i] = o
		...
		if err != nil {
			log.Println("No audio stream found in", src)
		} else {
			i, o, err := addStream("aac", octx, srcAudioStream, ictx)
			if err != nil {
				log.Fatal(err)
			}
			streamMap[i] = o
	...
	for {
		...
		allframes, err := ist.CodecCtx().Decode(pkt)
		if err != nil {
			log.Fatalf("error decoding - %s\n", err)
		}

		startTs := gmf.Rescale(trimStart*1000, int64(ist.TimeBase().AVR().Den), int64(ist.TimeBase().AVR().Num)) / 1000
		endTs := gmf.Rescale(trimEnd*1000, int64(ist.TimeBase().AVR().Den), int64(ist.TimeBase().AVR().Num)) / 1000
		...
		for i, frame := range frames {
			...
			frame.SetPts(frame.Pts() - startTs)
		}
